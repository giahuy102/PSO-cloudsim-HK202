...
public class TaskScheduler {
  ...
  public static double[] resultcost = new double[3];
  ...
  public static double[] func(int[] tasklength, int[] outputfilesize, int[] mips, double[] execcost,
    ...
    try {
      ...
      priorityScheduler(executiontimematrix, communicationtimematrix, datatransfermatrix, commcost,
  ...
  public static void SJFscheduler(double[][] executiontimematrix,
    ...
    try {
      ...
      for(Cloudlet cloudlet : cloudletList) {
        mapping_sjf[cloudlet.getCloudletId()] = index % Constants.NoOfVMs;
        index++;
      }
        ...
        for (int j = i + 1; j < Constants.NoOfTasks; j++) {
          if (taskoutputfilematrix[i][j] != 0) {
            delay_sjf[j] = Math.max(
            	delay_sjf[j], 
            	delay_sjf[i] + executiontimematrix[i][mapping_sjf[i]] + communicationtimematrix[i][j]
            );
          }
        }
      }
      ...
      for (int i = 0; i < Constants.NoOfVMs; i++)
        cost += vmworkingcost[i];
      for (int i = 0; i < Constants.NoOfTasks; i++) {
        cost += delay_sjf[i] * waitcost[mapping_sjf[i]];
      }
